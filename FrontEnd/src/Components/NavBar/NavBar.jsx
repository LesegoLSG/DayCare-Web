...
const NavBar = () => {
  ...
  const setActiveSectionFromURL = () => {
    const hash =
      location.hash[0] === "#" ? location.hash.slice(1) : location.hash; // Using array indexing
  ...
  // Handler to scroll to the home section
  const scrollToHome = () => {
          ...
          {sections.map((section) => (
            <li key={section} className="my-7 md:my-0 md:ml-8">
              <ScrollLink
                ...
                duration={500}
                // offset={-80}
                className={`text-gray-800 text-sm hover:text-gray-400 duration-500 cursor-pointer ${
