    ...
    <div className="flex flex-col justify-center items-center">
      <label htmlFor="file" style={{ cursor: 'pointer' }}>
        <div className="w-80 h-80 border rounded-full flex justify-center items-center">
          {image ? (
            <img className="w-full h-full object-cover rounded-full" src={URL.createObjectURL(image)} alt="Uploaded" />
          ) : (
            <img className="w-full h-full object-cover rounded-full" src={EmptyUser} alt="Default" />
          )}
        </div>
      </label>
      <input type="file" id="file" name="file" onChange={handleImage} style={{ display: 'none' }} />
